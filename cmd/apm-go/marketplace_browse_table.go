...
// lipgloss-boxed table (ux.Table). This intentionally does not reproduce the
// Python original's rich HEAVY_HEAD box styling byte-for-byte (design.md
// accepts the visual difference) -- only the Description column is still
// pre-wrapped here, since ux.Table itself does not word-wrap.
