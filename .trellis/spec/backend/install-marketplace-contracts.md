...
- `--ref` mutable value (branch / `HEAD` / short tag, i.e. not a 40-hex SHA) is resolved to a concrete SHA via `RefLister.ListRefs` before write (`resolveRef`); a 40-hex SHA is stored verbatim. **Updated (R5, 2026-07-30, task `07-29-marketplace-add-fixes`)**: an empty `--ref` on `add` is no longer a blanket short-circuit — it is treated as an implicit `HEAD` and resolved via `RefLister.ListRefs` like an explicit `--ref HEAD`, mirroring upstream `plugin/__init__.py:102-142`'s `is_head = ref is None or ref.upper() == "HEAD"` (`--no-verify` still hard-fails offline instead, exit 2). The **only** remaining short-circuit (no lister call) on `add` is mkt-046's local-source rule: a `./`-prefixed local source's ref is never resolved regardless of network access (`classifyRefResolution`, `editor.go`). `set` always resolves a given ref, including on a local source (no `--no-verify` escape hatch on `set`, and no mkt-046 local-source exemption either — `SetPackage` passes `skipLocalSource=false`).
