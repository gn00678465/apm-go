...
| `audit` | 有 | 有 | **✅ RESOLVED**（Phase 7；`--content` 補隱字掃描 pillar，bare SHA 零回歸；見 §3.1） | **high** | commit `f2453b8` | integrity §audit |
...
| `pack` | 有 | 有 | **✅ RESOLVED**（Phase 2-8；三 producer 修 parity + install 回路，test1 byte-identical；見 §3.2） | **HIGH** | commit `f547b4b`/`d1bfad8`/`6585876` | packaging §1 |
...
**✅ 已關閉（2026-07-13，task `07-12-p0-parity-quickwins` Phase 7）**：
以 flag 分流補齊缺失的隱字掃描 pillar，而非改 bare 行為。`apm-go audit`（bare）維持
SHA-256 重驗零回歸；新增 `apm-go audit --content` 走共用 `internal/security` ContentScanner
掃描已部署檔（跨 deps + local），exit 0/1/2。ContentScanner A/B：同一含 1 critical +
1 warning 的 fixture，`apm-go pack` 與 `apm pack` 皆報 2 hidden character(s)。
**drift 語意逐項核對**（`research/audit-drift-reconciliation.md`，Python `install/drift.py`
file:line 引用）：apm-go SHA 重驗覆蓋 Python 三分類的 `modified` 子集 + `unintegrated` 的
「已追蹤檔遭刪」子集；未覆蓋 never-recorded-but-should-exist 超集與 `orphaned` 全類
（需獨立 install-replay 引擎，非本輪）。`--help`/Long 明列不含 drift replay 與
`--ci`/`--policy`/`--external`/`--format`/`-o`/`--strip`。commit `f2453b8`。
...
**✅ 已關閉（2026-07-13，task `07-12-p0-parity-quickwins` Phase 2-8）**：
採「建議 1：修 parity」（非文件化止血）。三 producer 全數移植（BundleProducer +
PluginManifestProducer + 既有 MarketplaceProducer），`DetectOutputs` 三獨立觸發、
nothing-to-do 改 exit 1。**並補完整回路**：`install <bundle-path>` 消費子系統
（`internal/localbundle`）。Gate 6b test1 雙邊重播（見 `research/gate6b-report.md`）：
`apm-go pack` 與 `apm pack` 對 test1 產出 73 檔樹狀一致；`apm-go install` 與 `apm install`
部署樹路徑集 + 逐檔 sha256 **byte-identical**（222 檔，PATH_DIFF=0/HASH_DIFF=0）。
殘留 deviation（compile-only instruction staging、canvas extensions、pack target
metadata `all` vs `minimal`）於報告「此修正不做什麼」具體列出。commit `f547b4b`
（pack 三 producer）、`d1bfad8`（install 回路）、`6585876`（install verbatim parity 修正）。
...
執行 task：`.trellis/tasks/07-12-p0-parity-quickwins`（Gate 5 living-doc 更新，2026-07-12）。下表最後一欄記錄六項的處置狀態與證據；commit 欄位已於 2026-07-12 以實際 SHA `845944c` 回填（#6 純文件項的 spec 檔隨 docs commit 入庫）。

| # | 項目 | 動作 | 來源 | 處置狀態與證據 |
|---|---|---|---|---|
| 1 | `init.go:162` 虛假 `run` 提示 | 刪除或改寫該行，不承諾不存在的指令（1 行改動） | project §5 | 已完成——`cmd/apm/init.go` 移除 `apm-go run <script>` 這行提示，保留有效的 `apm-go install` next-step；測試 `TestInitCmd_DoesNotSuggestRun`（`cmd/apm/main_test.go`）以完整舊文案 `apm-go run <script>` 做負向斷言鎖定。commit: `845944c` |
| 2 | `pack` 靜默無輸出 | 在 `dependencies:`/`target:` 存在但 `marketplace:` 不存在時印警告，而非完全靜默 `nothing to do`；`--help`/README 註明範圍限定 | packaging §1.5 | 已完成——`cmd/apm/pack.go` 新增 `deferredPackInputs` 判斷 + `packDepsWarning`/`packTargetWarning` 兩則完整警告文案（有 deps/target 但無 marketplace 時印出，真正空專案仍維持 `nothing to do` 且不誤報）；`pack --help` Long 明示 marketplace.json-only 範圍、對照 Python plugin bundle。測試 `TestPackCmd_HelpDocumentsMarketplaceOnlyScope`、`TestRunPack_NoMarketplaceDeferredInput`（`cmd/apm/pack_test.go`），涵蓋 Gate 2 要求的排除輸入案例。commit: `845944c` |
| 3 | `audit` 語意落差 | `--help`/README/spec 明確記錄「apm-go audit ≠ Python apm audit（bare）」 | integrity §audit | 已完成——`cmd/apm/audit.go` `--help` Long 明示 apm-go audit 是 SHA-256 部署檔重驗、與 Python bare audit 的 hidden-Unicode 掃描不同；`.trellis/spec/backend/cli-parity-notes.md` 同步記錄相同對照。測試 `TestAuditCmd_HelpDocumentsSemanticDifference`（`cmd/apm/audit_test.go`）；audit 既有 hash-gate 行為（乾淨過/竄改失敗）未變動。commit: `845944c` |
| 4 | `approve`/`deny` 靜默失效 | 若 `apm.yml` 含 `allowExecutables` block，印警告「此區塊在 apm-go 尚不生效」 | integrity §approve/deny | 已完成——`internal/manifest/manifest.go` `ParseManifest` 新增 `allowExecutables` case，直接印出完整 `[warn]` 訊息並回傳對應 `Diagnostic`（install/update/uninstall/mcp install 等所有經過 `ParseManifest` 的路徑皆會看到，即使呼叫方目前丟棄回傳的 diags）；純提示、不改變部署行為，hooks/bin/MCP 仍無條件部署。測試 `TestParseManifest_AllowExecutablesWarning`/`TestParseManifest_NoAllowExecutables_NoWarning`（`internal/manifest/manifest_test.go`）與端對端 `TestRunInstall_AllowExecutablesWarning`（`cmd/apm/install_test.go`，證明 on/off 兩側部署 bytes 相同）。commit: `845944c` |
| 5 | `update` 無預覽 | 補 `--dry-run`（風險最低、獨立可做） | crosscut §2 D-1 | 已完成——`cmd/apm/update.go` 新增 `--dry-run` flag 與 `runUpdateDryRun`：解析改用拋棄式 scratch `ModulesDir`（絕不寫入真正的 `apm_modules`），沿用 `printUpdateSummary` 印出 `Update plan for apm.yml` 與新舊版本，不呼叫 `buildLockfile`/`deployAndFinalize`，因此不寫 `apm.lock.yaml`、不動 `apm_modules/`、不部署任何 target 檔案。測試 `TestUpdateCmd_DryRunFlag`（`cmd/apm/update_test.go`，鎖定 `--help` flag 文案）與 `TestRunUpdate_DryRunPlanNoSideEffects`（`cmd/apm/update_e2e_test.go`，真 git fixture 前後 byte-identical snapshot 比對 lockfile/apm_modules/`.claude`）。commit: `845944c` |
| 6 | `normalize`/`validate` 文件缺口 | 在 `.trellis/spec/backend/` 補記兩者為 dev-only CLI 化工具，並澄清 `validate` 與 `marketplace validate NAME`（MK-10）撞字面詞但範疇不同 | extensions §指令1/2 | 已完成——新增 `.trellis/spec/backend/cli-parity-notes.md`：記錄 `normalize`/`validate` 為 apm-go-only、dev-only CLI 化工具（EXTENSION），`normalize` 契約為 `yamlcore.SafeLoad`/`SafeDump` round-trip；並明確區分頂層 `validate <file>` 與 `marketplace validate NAME` 撞字但範疇不同（前者任意檔案、後者已註冊 marketplace 名稱）。純文件變更，不改變既有 CLI 行為；已於 `.trellis/spec/backend/index.md` 加入索引列。commit: `845944c` |
...
| 19a | **`pack` 完整 parity**（Gate 6a 補排，2026-07-12） | 移植缺失的 BundleProducer（`build/<name>-<ver>/` + 內嵌 lockfile）與 PluginManifestProducer（plugin.json 合成 + `.github/plugin/` + `.claude-plugin/`），與既有 marketplace.json producer 並存——P0 #2 的警告只是止血，本項才是 §3.2 DIVERGENT 的關閉條件。**✅ 完成（2026-07-13）：task `07-12-p0-parity-quickwins` Phase 2-8。三 producer 全移植 + `install <bundle-path>` 回路；Gate 6b test1 雙邊 byte-identical（見 `research/gate6b-report.md`）。commit `f547b4b`/`d1bfad8`/`6585876`** | packaging §1.5、§3.2；Gate 6a |
| 19b | **`audit` 完整 parity**（Gate 6a 補排，使用者 2026-07-12 拍板排入 P1） | 移植 Python bare audit 的隱藏 Unicode 掃描 + drift 偵測；與 apm-go 既有 SHA 完整性重驗的並存語意（子指令/flag 分流）於 research 定案——P0 #3 的文件說明只是止血，本項才是 §3.1 DIVERGENT 的關閉條件。**✅ 完成（2026-07-13）：task `07-12-p0-parity-quickwins` Phase 7。`audit --content` 補隱字掃描 pillar；drift 語意逐項核對於 `research/audit-drift-reconciliation.md`（未覆蓋的 drift 超集需 install-replay 引擎，明列於 Gate 6b 報告）。commit `f2453b8`** | integrity §audit、§3.1；Gate 6a |
### P2（長期，範圍大或需先決策）
