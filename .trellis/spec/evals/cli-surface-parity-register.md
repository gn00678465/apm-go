...
| 19a | **`pack` 完整 parity**（Gate 6a 補排，2026-07-12） | 移植缺失的 BundleProducer（`build/<name>-<ver>/` + 內嵌 lockfile）與 PluginManifestProducer（plugin.json 合成 + `.github/plugin/` + `.claude-plugin/`），與既有 marketplace.json producer 並存——P0 #2 的警告只是止血，本項才是 §3.2 DIVERGENT 的關閉條件。**執行中：task `07-12-p0-parity-quickwins` Phase 2a（2026-07-12 使用者裁定併回原 task，不另開）** | packaging §1.5、§3.2；Gate 6a |
| 19b | **`audit` 完整 parity**（Gate 6a 補排，使用者 2026-07-12 拍板排入 P1） | 移植 Python bare audit 的隱藏 Unicode 掃描 + drift 偵測；與 apm-go 既有 SHA 完整性重驗的並存語意（子指令/flag 分流）於 research 定案——P0 #3 的文件說明只是止血，本項才是 §3.1 DIVERGENT 的關閉條件。**已排程：task `07-12-p0-parity-quickwins` Phase 2b（2026-07-12 使用者裁定併回原 task，不另開）** | integrity §audit、§3.1；Gate 6a |
### P2（長期，範圍大或需先決策）
