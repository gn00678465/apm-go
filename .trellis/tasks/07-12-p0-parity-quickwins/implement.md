...
- [ ] `localbundle/detect.go`：`DetectLocalBundle`(目錄根 plugin.json/.zip/
      .tar.gz→BundleInfo；皆非→nil,nil)
- [ ] 測試：三形狀各 fixture；像本地依賴但無 plugin.json→nil,nil
- [ ] `localbundle/verify.go`：`VerifyBundleIntegrity`(無 lockfile→warn 不擋；
      symlink 一律拒；逐檔裸 hex；反向 unlisted-file 檢查)
- [ ] 測試：乾淨全過；竄改位元組→error 列路徑；symlink(列/未列各 fixture)→
      error；多 unlisted 檔→error；路徑穿越檔名→error
- [ ] `localbundle/integrate.go`：`IntegrateLocalBundle`(部署 plugin-native→
      resolved targets；零 target→warn 不失敗；check_target_mismatch 警告)
- [ ] 測試：正常部署；零 target；target mismatch 印警告仍部署
- [ ] `cmd/apm/install.go`：runInstall 最前插 DetectLocalBundle 分支→繞過
      resolver、verify+integrate、寫專案 lockfile local_deployed_*
- [ ] 測試：`install build/pkg-1.0.0/` 成功部署+lockfile；一般 install 不回歸；
