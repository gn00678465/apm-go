...
- [ ] R12c/d：audit/frozen 成功細節走 `--verbose`（預設不洗版）；R12e：compile 評估結果已記錄（做或再分流）。
      **R12e 決策（2026-07-16）：留本任務、已實作**——`compile.Result` 加只讀 `Sources []string`
      （既有 `CollectInstructions` 結果的投影，零重算、不改 compile 決策/序列化/跨 package API，
      符合 design §3 語意準則）；cmd 層預設印來源清單（instruction 數量小、對齊 Python 預設列出）。
...
- [x] R18：`.gitignore` 落差已記錄、決策補或不補。
      **決策（2026-07-16）：不補**。理由：(1) 建立/修改 `.gitignore` 是檔案系統行為變更，
      超出本任務 parity 呈現範疇（design §3）；(2) apm-go 在無 `apm_modules/` 的純 local
      安裝下該條目無實際作用；(3) Python 行為已記錄於 research/bug2-python-baseline.md
      （純 local 安裝亦印 `Added apm_modules/ to .gitignore`）。若日後補，另立行為需求
      （idempotency、既有內容保留、換行格式）。
