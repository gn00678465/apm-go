...
- SC-F13 `VersionTagCandidates_VPrefixedCapture`：
  ...
  - 子測試 fallback-infers（回歸）：pattern `{version}`，tags 只有 `v1.2.0`。expect 候選為 `v1.2.0`（version `1.2.0`），usedPattern 為 `v{version}`（oracle `__init__.py:979-990` 推斷；實跑 `infer_tag_pattern("v1.2.0")="v{version}"`）。
