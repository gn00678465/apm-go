...
| Fields | `'<field>' must be a string` / `… must be an object` / `… must be a boolean` / `… must be an array of strings` / `… must be a string or array` / `… must be a string, array, or object` / `'author.<k>' must be a string` / `'dependencies[<i>]' must be a string or object` / `'dependencies[<i>]' is missing 'name'` / `'dependencies[<i>].version' must be a string` / `'metadata' should be an object; Claude Code ignores other values`（warning）/ `'experimental' should be an object; Claude Code ignores other values`（warning）/ `'experimental.<k>' must be a string or array` |
