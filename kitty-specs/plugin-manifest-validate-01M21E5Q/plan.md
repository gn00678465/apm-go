...
| Gate 2：輸出契約；任何使用者可見輸出／exit code／產生檔的變更要加或改 corpus case | **需裁定** | 這是 apm-go 獨有指令，Oracle 沒有對應命令，`tools/parity` 無法產生 Oracle 側輸出。處置：不加 parity case；改以 `tools/gate/realexec.sh` 固定 stdout / exit code 為輸出契約證據，並在 `cmd/apm-go/plugin.go` 的偏差註記說明「無 Oracle 對應、契約由 realexec 固定」。既有 96 case 必須維持 0 unwaived。 |
...
唯一的「需裁定」項是 Gate 2 對無 Oracle 對應指令的適用方式；本計畫採「realexec 固定契約 + 偏差註記」，reviewer 可在 review 時否決。
