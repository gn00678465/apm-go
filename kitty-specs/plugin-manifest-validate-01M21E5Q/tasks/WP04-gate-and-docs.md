...
Success criteria:
- `tools/gate/realexec.sh` has 2 new happy-path steps and 4 new adversarial steps for `plugin validate`, each pinned at ticket 34's verification strength: the exact exit code, the complete stdout compared against a recorded expectation (not a substring), stderr asserted empty, and a recursive before/after comparison of the fixture tree proving the run is read-only.
- `tools/gate/realexec.sh` gains the two helpers that strength needs: one that captures stdout and stderr into separate files (the existing `step` merges them with `2>&1`, so it cannot assert an empty stderr), and one that compares a captured stream byte-for-byte against an expected file. Ticket 34 (.scratch/parity-runner/issues/) is the authority on why substring checks are not enough here.
...
- **Steps**:
  ...
  2. Compare the step's complete stdout against a recorded expectation transcript (its exact shape is fixed by `contracts/cli-plugin-validate.md`, including the ` i Summary: N passed, 0 warnings, 0 errors` line), and assert the stderr capture is empty. A `must_grep` containment check is not sufficient -- ticket 34 requires the whole stream.
  ...
  4. Add a recursive read-only check immediately after each of these two steps, comparing the whole fixture directory (not only the manifest file) before and after the `plugin validate` invocation (save a copy right before the step, per this file's existing `cp apm.yml "$SB/mk-apm.yml.before"` convention seen later in the adversarial section) -- confirm byte-identity, incrementing `steps`/`failures` the same way the existing final `cmp` check in this file does.
...
- **Steps**:
  1. **Invalid JSON**: create a directory with a deliberately broken `plugin.json` (e.g. `printf '{' > x/plugin.json`, matching quickstart.md's own "broken" example), then `step plugin-validate-invalid-json 1 "$BIN" plugin validate x`, and compare its complete stdout against a recorded expectation whose error line carries `invalid JSON`, with stderr asserted empty.
  2. **Path traversal field**: a `plugin.json` with a path-typed field value escaping the plugin root (e.g. `{"name":"x","commands":"../outside/x.md"}`, per spec.md US1 AS6) -- `step plugin-validate-traversal 1 "$BIN" plugin validate y`, compared as a complete stdout expectation whose error line is the literal `must not contain '..'` message, with stderr asserted empty.
  3. **Typo'd field with --strict**: a `plugin.json` with `{"name":"x","descripton":"d"}` (per spec.md US2 AS1-AS2) -- `step plugin-validate-strict-typo 1 "$BIN" plugin validate z --strict`, compared as a complete stdout expectation that includes the unrecognized-field warning and its `description` suggestion, with stderr asserted empty.
  4. **Missing manifest**: an empty directory with none of the four candidate files -- `step plugin-validate-missing 1 "$BIN" plugin validate empty`, compared as a complete stdout expectation carrying the `no plugin.json found` message, with stderr asserted empty.
  5. For EACH of the four steps above, snapshot the whole fixture directory and compare it recursively before and after the step (byte comparison of every file, plus the directory listing itself so a created or deleted file is caught; the missing-manifest case compares an empty tree to an empty tree) immediately around the `step` call, following this file's own existing `cmp -s apm.yml "$SB/mk-apm.yml.before"` pattern (near the end of the adversarial section) rather than inventing a new verification style.
- **Files**: `tools/gate/realexec.sh` (existing).
...
- Capture the four new adversarial steps' expected transcripts from the actual binary once, outside the full gate run, if iterating quickly (`cd .gate/<scope>/sandbox/work && ../../../bin/apm-go plugin validate ...`) before committing to a full gate run each time.
