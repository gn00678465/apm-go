...
- **Steps**:
  ...
  6. SUPERSEDED by the final contract: the size is checked on the same open handle rather than by a second path-based `Stat`, and the actual read is bounded to the cap plus one byte, so a file that grows or is swapped after the check cannot be read past the cap. Over the cap the command reports `could not read '<path>': <reason>` with no Results or Summary and exit 1, not a Structure finding. `Validate`'s own length check remains a second line of defense for direct callers.
