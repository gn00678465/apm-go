...
- `internal/pack/bundle/producer.go` lines 489-497 (`pluginJSONCandidates`, read-only reference -- do NOT edit this file, it is outside this WP's `owned_files` and belongs to a different package with its own ownership) -- the four-candidate probe order and its doc comment citing `utils/helpers.py:105-129`. This WP duplicates that same four-string list locally in `plugin_validate.go` with a doc comment citing this exact location as the source of truth, rather than exporting a new symbol from `bundle` (avoids a new cross-package dependency edge not in ARCHITECTURE.md §1, per C-002; matches the existing precedent of small, documented duplication seen in `internal/pack/bundle/schema_sync_test.go`'s own doc comments).
...
- **Steps**:
  1. In new `cmd/apm-go/plugin_validate.go`, declare a local `var pluginValidateCandidates = []string{"plugin.json", filepath.Join(".github", "plugin", "plugin.json"), filepath.Join(".claude-plugin", "plugin.json"), filepath.Join(".cursor-plugin", "plugin.json")}` with a doc comment: "mirrors `internal/pack/bundle/producer.go`'s `pluginJSONCandidates` (upstream `find_plugin_json`, `utils/helpers.py:105-129`) -- duplicated locally rather than exported across packages, per this WP's prompt's own C-002 rationale."
  ...
  5. Once a candidate is selected, `Lstat` its resolved path once more if it is a symlink: resolve the link target and confirm it stays within `path`'s tree (NFR-003 "不跟隨 symlink 到 path 之外"); if it escapes, treat this identically to "no plugin.json found" (do not read through it).
...
- **Files**: `cmd/apm-go/plugin_validate.go` (new).
...
- **Notes**: Re-read the contract's worked example block closely (the ` ! Fields: ...` / ` x Paths: ...` / ` ! Unrecognized: ...` lines) -- error lines and warning lines interleave ACROSS checks in file order (Structure, Name, Fields, Paths, Unrecognized), but WITHIN a single check's own findings, errors print before warnings (matching data-model.md's Findings ordering: "依 Check 順序、同 Check 內依檔案順序" -- note this is file-order within a check, not error-then-warning; re-verify against WP01's actual `Report.Findings` ordering once WP01 ships, since data-model.md's phrasing describes storage order, while `marketplace validate`'s rendering convention this WP mirrors sorts errors-before-warnings at render time regardless of storage order -- follow the render-time convention `marketplace validate` uses, matching this contract's own worked example where Fields shows only a warning and Paths shows only an error, so the two convention are consistent in the example; if WP01 ever emits BOTH an error and a warning in the same check, render errors first within that check, mirroring `marketplace.go`'s own two-pass filter).
