...
- **Steps**:
  ...
  8. Append findings so that, within each check, errors precede warnings and each group keeps file order (data-model.md `Report.Findings`); WP03 renders the slice as-is. Cover this with one test where a single check emits both an error and a warning.
