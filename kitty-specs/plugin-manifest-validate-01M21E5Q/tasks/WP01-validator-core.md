...
- **Steps**:
  ...
  7. Populate `Report.KnownFieldsPresent` (FR-011, consumed by WP03's `-v` rendering): reuse the same token-order scan from step 5 to collect top-level keys in file order, keep only those present in the rule table (top-level entries; `experimental.*` sub-keys are not listed), de-duplicate on repeated keys, and leave it empty when Structure failed before decoding. Add a unit test asserting file order and exclusion of unrecognized keys.
  8. Append findings so that, within each check, errors precede warnings and each group keeps file order (data-model.md `Report.Findings`); WP03 renders the slice as-is. Cover this with one test on the Fields check -- a manifest carrying both a type-error field and a non-object `metadata` puts an error and a warning in the same check. Structure cannot produce that mix: step 6 short-circuits before a duplicate-key warning could sit beside a Structure error.
...
- **NFR-003 note**: this WP satisfies NFR-003 (read only the selected manifest, no symlink following) trivially: `Validate` takes bytes and performs no filesystem access at all. The symlink/parent-directory logic lives entirely in WP03's T013.
