...
// SetTTYSeamsForTest overrides the stdin/stderr TTY-detection seams that
// back CanPrompt (and Init's richMode calculation) for the duration of a
// test in another package, where the real seams (stdinIsTTY/stderrIsTTY)
// are unexported. It returns a restore func that must be invoked (e.g. via
// t.Cleanup) to put the originals back.
//
