...
| `marketplace/tagpattern` | tag-pattern validation, compilation, filtering, common-layout inference | `Validate` / `Compile` / `FilterTags` `internal/marketplace/tagpattern/tagpattern.go:45,77,136`; `Infer` `:149` |
...
**`marketplace check` / `marketplace outdated`** (`cmd/apm-go/marketplace_authoring.go`): `authoring.LoadAuthoringConfig` (a validation error exits 2 via `configLoadError`, `cmd/apm-go/marketplace_authoring.go:383`) → one `RefLister.ListRefs` per remote package → `authoring.CheckPackagesWith` (`internal/marketplace/authoring/refcheck_sha.go:88`; per entry `checkPackage`, `refcheck.go:796`) with the three seams `CheckDeps{Lister, Prober, Manifest}`, whose production git work runs in `fetchRefIntoScratch` / `showAtFetchHead` (`refcheck_sha.go:189,286`); `outdated` goes through `outdatedForPackage` (`refcheck.go:999`): a lowercase 40-hex pin that no advertised ref's commit matches is probed with `git fetch --depth 1` into a scratch bare repo, and a pin carrying a display version is compared against the plugin manifest at that ref; `outdated` compares a SHA pin against release tags (display version) or the default-branch tip (no version) and skips every other ref pin. Both share `versionTagCandidates` (`refcheck.go:1171`) (tags only, semver-valid, prerelease opt-in, `tagpattern.Infer` fallback). The two commands have no parity corpus case (remote sources cannot be local fixtures); `tools/gate/realexec.sh` pins their network-free contract (schema errors, `--offline`, local packages).
...
**gitops secure environment.** Every git subprocess runs under `gitops.ApplySecureGitEnv` (`internal/gitops/secure_env.go:53`: credential prompts off, `GIT_ALLOW_PROTOCOL` allow-list, `GIT_PROTOCOL_FROM_USER=0`) or the clone-specific `ApplyCloneEnv` (`harden.go:68`). Call sites: `gitops/clone.go`, `gitops/tags.go`, `cmd/apm-go/doctor.go:60`, `marketplace/client_git.go:83,92`, `marketplace/authoring/refcheck.go:97`, `marketplace/authoring/refcheck_sha.go:161,211,290` (scratch fetch / init / show, each also pinned to `LC_ALL=C`), `marketplace/build/metadata.go:157-170`, `marketplace/build/reflister.go:83`, `tools/parity/preflight.go:290`. Git stderr reaches the user only through `SanitizeGitOutput` / `TranslateGitStderr` (`sanitize.go:17`, `stderr.go:70`).
