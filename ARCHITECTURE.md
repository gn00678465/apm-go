...
**`marketplace check` / `marketplace outdated`** (`cmd/apm-go/marketplace_authoring.go`): `authoring.LoadAuthoringConfig` (a validation error exits 2 via `configLoadError`) → one `RefLister.ListRefs` per remote package → `authoring.CheckPackagesWith` with the three seams `CheckDeps{Lister, Prober, Manifest}` (`internal/marketplace/authoring/refcheck_sha.go`): a lowercase 40-hex pin that no advertised ref's commit matches is probed with `git fetch --depth 1` into a scratch bare repo, and a pin carrying a display version is compared against the plugin manifest at that ref; `outdated` compares a SHA pin against release tags (display version) or the default-branch tip (no version) and skips every other ref pin. Both share `versionTagCandidates` (tags only, semver-valid, prerelease opt-in, `tagpattern.Infer` fallback). The two commands have no parity corpus case (remote sources cannot be local fixtures); `tools/gate/realexec.sh` pins their network-free contract (schema errors, `--offline`, local packages).
