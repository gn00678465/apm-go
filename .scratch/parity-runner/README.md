...
| 31 | Stream glyph audit and channel contract | 10 |
| 32 | Command-semantics gaps from the ticket-31 audit | 31 |
| 33 | Stream TUI symbols + parity glyph-shape normalization | 31 |

Frontier order: {01, 04} → {02, 03} → {06, 07, 08, 10} → {05} → 09.

Status: verification gates are GREEN for ticket 33 (2026-08-29). Tickets 31 and 33 are CLOSED; ticket 32 remains open for its five command-semantics cases. The pinned Oracle is `b75a02b1cfab3ffa5e1952916045b6d5374090ae` (v0.29.0); the corpus is **96 cases, 0 unwaived** after always-on normalization of Oracle bracket glyphs and apm-go's centered TUI glyphs. `go vet ./...` and `CI=true GITHUB_ACTIONS=true go test ./... -race` are the required gates. Ticket 30 is a documented apm-go-only `--claude-source-style url` flag; the default remains Oracle-compatible. CI-detection env remains pinned in TestMain so ambient `CI`/`GITHUB_ACTIONS` cannot change local-vs-Actions results.

Tickets 01-31 and 33 are closed, with 23 CLOSED INVALID; ticket 32 remains open for its five command-semantics cases. The 18-case backlog was triaged into root causes rather than symptoms and cleared: 17 (all 8 missing `pack` flags across 5 phases), 18 (audit verbose hint), 22 (validate `[*]`/`[+]` glyphs + single quotes), 23 (the Oracle's `apm marketplace search` hint names a command that was never registered), 24 (`marketplaces.json` `file://` form + materialise-on-read, 11 cases' tree), 25 (rendering-class waivers), 26 (`marketplace list`'s missing "Registered Marketplaces" title and ALL-CAPS headers), 27 (bundle lockfile's `generated_at`/`deployments`), 28 (marketplace success catalog, absolute paths, and Claude key order), 29 (Codex category-required wording), and 30 (the documented apm-go-only HTTPS GitHub source-style flag). Ticket 33 applies the user ruling that apm-go emits centered width-3 TUI symbols (`+`, `i`, `!`, `x`, `>`, `*`) and that parity always normalizes glyph shape; brackets are never emitted by apm-go.
