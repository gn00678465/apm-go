...
Status: verification gates are GREEN (2026-08-28). The pinned Oracle is now `b75a02b1cfab3ffa5e1952916045b6d5374090ae` (v0.29.0); the 80-case corpus remains **80 cases, 0 unwaived**, unchanged across the pin bump. `go vet ./...` and `CI=true GITHUB_ACTIONS=true go test ./... -race` pass. Ticket 30 is a documented apm-go-only `--claude-source-style url` flag; the default remains Oracle-compatible. CI-detection env remains pinned in TestMain so ambient `CI`/`GITHUB_ACTIONS` cannot change local-vs-Actions results.
