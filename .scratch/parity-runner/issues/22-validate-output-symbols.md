...
- [ ] AC1 — Cite the Oracle file:line for both the header's `logger.success(..., symbol=...)` call and the per-check rows' call, and confirm from the source (not from the diff alone) which symbol each one actually resolves to. If the source contradicts the observed bytes, the source wins and this ticket's premise is wrong — say so rather than forcing the bytes to match.
- [ ] AC2 — Header line renders `[*] Validating marketplace 'NAME'...` — `[*]` glyph, single quotes.
- [ ] AC3 — Per-check pass rows render `[+]   Structure: passed` (and Schema/Names), preserving the existing three-space indent exactly. The existing `[x]` failure row must not change.
- [ ] AC4 — Do **not** widen `ux.Success` → `ux.Sparkle` globally. Ticket 13 deliberately scoped `Sparkle` to verified call sites; change only the call sites this ticket's evidence covers.
- [ ] AC5 — `validate-checkrefs-off`, `validate-checkrefs-on`, `validate-structure-fail` each report **no `stdout` and no `error_body` diff** in a fresh runner corpus. Their remaining `tree` diff is ticket 24's and their `stderr` diff is out of scope (see below) — those may stay.
- [ ] AC6 — Zero regression elsewhere: every other case's `(fields, waived)` tuple is unchanged from the pre-change baseline.
- [ ] AC7 — Go tests asserting the exact bytes of both line shapes.
