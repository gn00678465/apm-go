...
**Status:** closed (PASS at attempt 3 — wording fixes stand from attempt 1; attempt 2's Rich-wrap emulation was withdrawn per orchestrator direction, see "Attempt 3" below)
...
## Attempt 3 (orchestrator direction change: withdraw the wrap experiment)

Attempt 2's rewrite fixed all four reproducers the evaluator raised, verified against both live Oracle probes and a checked-in conformance fixture generated from the Oracle's own `rich` library. But the underlying pattern was the ticket-11 pattern repeating: chasing byte-exact fidelity to an upstream rendering library turns into an unbounded emulation surface. Four real renderer defects (effective width detection, cell width, long-word folding, hard-newline handling) surfaced in a *single* evaluator review round, and the same shape of gap plainly extends further behind them — Unicode digit forms in `COLUMNS` (Python's `str.isdigit()` accepts far more than ASCII 0-9), ZWJ/variation-selector grapheme clustering (`rich.cells.split_graphemes`'s SPECIAL handling, explicitly *not* ported), tab-stop expansion (`Text.expand_tabs`, called before wrapping and never implemented here at all), and ANSI-escape-sequence passthrough are all reachable inputs a determined evaluator round could produce next. This is precisely eval-plan §8.3's own governing distinction: Rich-vs-ux/lipgloss rendering-library differences are compare-semantics-and-waive territory (the doctor-healthy/search-basic-hit/validate-checkrefs-off precedent already used throughout this whole runner corpus for box-drawing, title alignment, and per-cell wrapping), not chase-byte-parity territory. apm-go never wrapped output before this ticket, and no product requirement asks it to emulate a terminal-width-aware renderer — a fifth evaluator round chasing a fifth reproducer class would not be closing a real gap in apm-go's product behavior, it would be re-implementing Rich.

**Decision, going forward:** terminal-width word-wrap emulation is explicitly OUT of apm-go's product scope. Single-line messages (one message, one line, regardless of terminal width) are apm-go's own UX contract — the Oracle's Rich-console wrapping of a long line is treated exactly like its box-drawing style or centered titles: a rendering-library difference to compare-and-waive, not a byte-parity target.

### Revert

- Deleted `internal/ux/wrap.go`, `internal/ux/wrap_test.go`, `internal/ux/rich_wrap_conformance_test.go`, `spec/conformance/rich-wrap.json`, `tools/rich_wrap_conformance_gen.py`.
- `internal/ux/printer.go`'s `oracleLine` reverted to its pre-ticket-14 body (`"<prefix><message>"`, one line, no wrap call), with a doc comment recording that the wrap experiment was tried and withdrawn, and why (this file, Attempt 3).
- `go.mod`'s `github.com/mattn/go-runewidth` dropped back to an indirect, transitive-only dependency (`go mod tidy`) — it was only ever imported by the now-deleted `wrap.go`.
- The ticket's actual acceptance-criteria work is untouched: `marketplaceNotRegisteredErr(verb, name)`'s Oracle-fixed-format rewrite (all 5 call sites), the `marketplace list` empty-registry sentence, and their tests all stand exactly as attempt 1 left them, still PASSING per the evaluator's own review.
- Test fallout from the wrap's removal reverted too: `containsUnwrapped`/`containsUnwrappedPath` (`cmd/apm-go/wraptest_helpers_test.go`, added across attempts 1-2 specifically to tolerate wrap-inserted newlines) deleted, and every call site (`marketplace_e2e_test.go`, `pack_test.go`, `search_test.go`, `uninstall_test.go`, `uninstall_antigravity_test.go`, `uninstall_summary_test.go`) reverted to plain `strings.Contains` — apm-go's output is single-line again, so there is nothing to tolerate.

### Waivers

- **Restored the 5 `pack-*` `stdout` waivers** (`pack-no-flag`, `pack-claude-plugin-flag`, `pack-format-claude`, `pack-format-claude-plugin`, `pack-format-plugin`) verbatim from their pre-ticket-14 text (ticket 13's own doctor-healthy-precedent reason: the Oracle's "Claude plugin bundle ready -- ..." line word-wraps, apm-go prints it on one line), each with one added sentence noting they were briefly stale under attempt 1/2's wrap and are now restored unchanged. A fresh corpus run confirms all 5 differ again on exactly `stdout`, nothing else.
- **Rewrote `browse-unknown-marketplace`/`list-empty`/`search-unknown-marketplace`'s waivers** to name the real (single-line-vs-wrapped) mechanism instead of attempt 2's wrap-length-shift one:
  - `browse-unknown-marketplace`: `fields: [stdout, error_body]`. Verified `error_body`'s normalized wording is byte-identical content, but Rich's own wrap ends the Oracle's *first line* at the first sentence while apm-go's single unwrapped line contains the whole message — `error_body`'s "first non-preamble line" extraction therefore captures a different AMOUNT of the same content, not different content.
  - `list-empty`: `fields: [stdout]` only — verified this exit-0 success-path case never populates `error_body` at all (only `stdout` differs, confirmed by inspecting its raw diff).
  - `search-unknown-marketplace`: `fields: [stdout, error_body]`, same mechanism as browse's, applied to `search.go`'s own separate "not registered" string (ticket 05's wording territory, not touched here) — recorded only because it shares the identical >80-cell-message-plus-no-wrap shape, not because it's one of this ticket's two written findings.
  - None of the three include `tree` in `fields`, matching the established `search-basic-hit`/`validate-checkrefs-off` convention: the ticket-05/12 registry-serialization gap on `home/.apm/marketplaces.json` stays deliberately unwaived on its own, unrelated to rendering.
- `tools/parity/main_test.go`'s `TestRealWaiversJSON_ValidatesAgainstPin` hardcoded id list updated to restore the 5 `pack-*` ids at their original position.

### Evidence (attempt 3)

Fresh clean-tree corpus (49/49 cases): diffed against the evaluator's own independent pull of attempt 2's evidence (`/tmp/ticket14-evidence-e2d62f2-clean2`), the drift is exactly: (a) `browse-unknown-marketplace` gains `error_body` (now waiver-documented), (b) the 5 `pack-*` cases return to `fields: [stdout], waived: true` (their restored waiver applying cleanly), (c) `search-empty-marketplace`/`search-empty-query`/`search-last-at-split`/`search-zero-results` (four OTHER search cases, none of which are this ticket's targets) return to showing their `stdout`/`error_body`/`tree` diffs again. Cross-checked against `/tmp/ticket13-evidence-a0b3f82-clean` (the evidence from immediately before ticket 14 ever touched anything): all four of those search cases are now byte-identical `(fields, waived)` to their original pre-ticket-14 state — confirming the revert is complete and these are ticket 05's pre-existing, untouched territory, not a new regression. The aggregate unwaived count returns to 18/49, matching the original pre-ticket-14-attempt-2 baseline exactly. `go test ./...` passes except the pre-existing, documented Windows-only `TestParseDepString_AbsolutePath`. `go test ./cmd/apm-go/... ./internal/ux/... ./tools/parity/... -race -count=1` passes clean.
