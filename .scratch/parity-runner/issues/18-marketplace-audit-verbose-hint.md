...
**Blocked by:** none. **Status:** CLOSED (2026-08-28) (spun out of ticket 14 attempt 2's evaluator review).
...
- [x] Confirm live against the pinned Oracle whether the hint line is audit-specific or shared, and cite the exact oracle file:line.
- [x] Add apm-go's own runner case (`marketplace audit <unregistered-name>`) if one doesn't already exist, capturing this exact gap.
- [x] Implement the hint line at the verified call site(s) only.
- [x] Fresh corpus evidence: the new/updated case shows no unwaived diff on this specific line; zero `(fields, waived)` regression elsewhere.

## Outcome (2026-08-28)

The hint is audit-specific: the Oracle's `commands/marketplace/audit.py:140-143` catch-all handler calls `logger.info("Run with --verbose for details.")` at line 142 after rendering the caught error. Strict-count failures exit through `SystemExit` before that handler and correctly omit the hint. apm-go now mirrors those two paths in `cmd/apm-go/marketplace_authoring_audit.go`, covered by `cmd/apm-go/marketplace_audit_verbose_hint_test.go`.

The new `marketplace-audit-unregistered` parity case leaves only the accepted Rich line-wrap residue on `stdout`; `error_body` is equal after mechanical continuation-line folding and no unwaived tree difference remains. That stdout field is recorded as a field-scoped waiver with byte-proof wording in `tools/parity/waivers.json`.
